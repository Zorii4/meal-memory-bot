...
Статус: реализуется, требования актуализированы
Дата: 2026-07-21, обновлено 2026-08-13
...
- для разрешённого пользователя показывает короткое приветствие;
...
1. Пользователь нажимает `🍽 Посоветовать блюдо`.
2. Если база пуста, бот предлагает сначала добавить несколько блюд.
3. Приложение формирует до пяти кандидатов, которые не готовили дольше всего.
4. Приложение передаёт ИИ только этот шорт-лист, недавнюю историю и названия блюд каталога.
5. ИИ:
...
6. Приложение валидирует ответ ИИ.
7. Бот присылает рекомендацию и inline-кнопки.

Пример ответа:

```text
🍽 Сегодня: чечевичный суп

Вы давно его не готовили, а после нескольких мясных блюд бобовые добавят разнообразия источников белка и клетчатки.

✨ Похожая новинка: суп с нутом и овощами
...
- `🔄 Другой совет` — формирует следующий совет, не меняя историю приготовления.
...
Действие `✨ Похожий совет` у конкретного блюда в `📚 Мои блюда` — отдельный сценарий, а не рекомендация «что приготовить сегодня».

1. Пользователь выбирает знакомое блюдо в каталоге.
2. Приложение передаёт ИИ выбранное блюдо как единственный допустимый контекст для похожей идеи, а также ограниченную недавнюю историю и названия активного каталога.
3. ИИ вызывается ровно один раз и предлагает одну валидную новинку, которой ещё нет в каталоге.
4. Бот показывает только новинку и краткое объяснение сходства. Исходное блюдо не выводится как `🍽 Сегодня`, а объяснение выбора основного блюда не показывается.
5. Для новинки доступны только действия:
   - `✅ Приготовили новинку` — сохранить и создать `cook_event`;
   - `💾 Сохранить новинку` — сохранить без `cook_event`.
6. Поиск похожей идеи сам по себе не создаёт `cook_event` и не влияет на давность или историю обычных рекомендаций «что приготовить сегодня».
7. Если ИИ недоступен, вернул невалидный ответ или не предложил допустимую новинку, бот сообщает, что похожее блюдо подобрать не удалось. Исходное блюдо не используется как fallback-ответ.
...
5. Если в базе достаточно вариантов, исключить два последних рекомендованных блюда, чтобы кнопка `Другой совет` не зацикливалась.
...
Индекс: `(created_at DESC)`.

Для существующей базы `purpose` добавляется новой миграцией со значением `daily` для старых записей. Событие `similar` можно сохранять как контекст callback-кнопок новинки, но оно не должно менять `lastRecommendedAt` обычного совета.
...
- Единственная публичная точка: `POST /telegram/webhook`.
...
Структурированный JSON-лог на каждую значимую операцию:

- `requestId`;
- `event`;
- `telegramUpdateId`;
- обезличенный или допустимый внутренний user marker;
- `durationMs`;
- `aiProviderStatus`;
- `fallbackUsed`;
- `errorCode` без секретов и содержимого prompt.

Пользователю показывается понятное сообщение, технические подробности остаются в логах.
...
1. Команда просмотра, архивирования и полного удаления блюда из общего списка. Полное удаление требует явного подтверждения и удаляет связанные с блюдом записи истории и рекомендаций, чтобы не оставлять несогласованные данные.
2. Быстрые фильтры: завтрак, суп, быстро, без мяса.
3. Уточнение доступных продуктов «что есть дома».
4. Личные предпочтения каждого из двух пользователей.
5. Недельное меню и список покупок.
6. Явные аллергены и ограничения после отдельного согласования.
7. Cloudflare Queue, только если ИИ регулярно не укладывается в таймаут.
