...
import { getCachedBotInfo } from "./bot/bot-info-cache";
...
import { D1UserGuideRepository } from "./infrastructure/d1/user-guide-repository";
...
async function processTelegramUpdate(update: unknown, env: WorkerEnv): Promise<void> {
  ...
  const userGuides = new D1UserGuideRepository(env.DB);
  const ai = new AIClient(config.ai);
  const botInfo = await getCachedBotInfo(config.telegram.botToken);
